...
class AdminController extends \SPUI\Controller
{
    ...
    public function handleImageUploadHook($meta, $id)
    {
        ...
        $mediaItem = $fs->getImage($id, 'media');
				...
				if ($mediaItem->getExtension()  == 'pdf')
				{
					...
					{
						 Log::addDebug('Image Upload Hook detected PDF, which is turned off - not optimization');
						 return $meta;
					}
				}
        ...
        /* if ($upscaleAiController->isAutoAiEnabled())
         {
    ...
    */
    public function filter_listener($query)
    {
      ...
      $filter = $this->selected_filter_value( 'spui_status', 'all' );
...
//  		add_filter( 'posts_orderby', array( $this, 'query_add_orderby' ), 10, 2 );
    ...
    public function filter_add_where ($where, $query)
    {
        ...
        switch($filter)
        {
             ...
             case 'prevented':
                ...
                $where = $wpdb->prepare($sql, '_spui_prevent_upscale');
            break;
        }
    ...
    */
    ...
    public function generatePluginLinks($links) {
        $in = '<a href="options-general.php?page=wp-shortpixel-upscale-settings">Settings</a>';
    ...
    */
    public function addMimes($mimes)
    {
		...
		/** Media library gallery view, attempt to add fields that looks like the SPUI status */
      ...
      return;
				...
				$fields["shortpixel-upscale-image"] = array(
    ...
    */
    public function toolbar_spui_processing( $wp_admin_bar ) {
        ...
        $tooltip = __('ShortPixel upscaling...','shortpixel-upscale-image');
        $icon = "shortpixel.png";
        $successLink = $link = admin_url(current_user_can( 'edit_others_posts')? 'upload.php?page=wp-shortpixel-upscale-bulk' : 'upload.php');
        $blank = "";
        ...
        if($quotaController->hasQuota() === false)
            ...
            if ($access->userIsAllowed('quota-warning'))
            {
              ...
              $link = "options-general.php?page=wp-shortpixel-upscale-settings";
            }
            else {
              $exceedTooltip = __('ShortPixel quota exceeded. Click for details.','shortpixel-upscale-image');
              //$link = "http://shortpixel.com/login/" . $this->_settings->apiKey;
    ...
    //            'href'  => 'javascript:void(0)', // $link,
        ...
        if($quotaController->hasQuota() === false)
            ...
            $wp_admin_bar->add_node( array(
                'id'    => 'spui_processing-title',
                'parent' => 'spui_processing',
                'title' => $exceedTooltip,
                'href'  => $link
            ));
