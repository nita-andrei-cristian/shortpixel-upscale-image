...
class CDNController extends \SPUI\Controller\Front\PageConverter
{
	...
	protected function createArguments($args = [])
	{
		...
		// Depend this on the SPUI setting
	...
	protected function loadCDNDomain($CDNDomain = false)
	{
		...
		{
			 $cdn_domain = trailingslashit($cdn_domain) . 'spui/'; 
		}
	/*	elseif ($parsed_domain['path'] !== '/spui')
		{
			 $cdn_domain = $parsed_domain['scheme'] . '://' . $parsed_domain['host'] . '/spui'; 
		} */
	...
	protected function fetchImageMatches($content, $args = [])
	{
		...
		// Updated pattern via - https://github.com/short-pixel-optimizer/shortpixel-upscale-image/issues/159
