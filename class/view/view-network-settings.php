...
<h1>
    ...
    <?php esc_html_e('ShortPixel Network Settings','shortpixel-upscale-image');?>
</h1>
            ...
            <content>
              <?php $this->printSwitchButton(
                    ['name' => 'delivery_enable',
                     ...
                     'label' => esc_html__('Enable site-wide settings','shortpixel-upscale-image')
                    ]);
              ?>
            ...
            <ul class="deliverWebpTypes toggleTarget" id="deliverTypes">
                <li>
                    ...
                    <?php if($deliverWebpAlteredDisabledNotice){ ?>
                        <p class="sp-notice">
                            <?php esc_html_e('After the option to work on .htaccess was selected, the .htaccess file has become unaccessible / read-only. Please make the .htaccess file writeable again to be able to further set this option up.','shortpixel-upscale-image')?>
                        </p>
                    <?php } ?>

                    <p class="settings-info">
                         <?php esc_html_e('Each &lt;img&gt; will be replaced with a &lt;picture&gt; tag that will also provide AVIF and WebP images for browsers that support it.  You don\'t need to activate this if you\'re using the Cache Enabler plugin because your AVIF\WebP images are already handled by this plugin. <strong>Please run some tests before using this option!</strong> If the styles that your theme is using rely on the position of your &lt;img&gt; tags, you may experience display problems.','shortpixel-upscale-image'); ?>
                        <strong><?php esc_html_e('You can revert anytime to the previous state just by deactivating the option.','shortpixel-upscale-image'); ?></strong>
                    </p>
                ...
                <li>
                    ...
                    <?php if ( strlen( $deliverWebpUnalteredLabel ) ) { ?>
                        <p class="sp-notice sp-notice-warning"><strong>
                            <?php echo wp_kses_post( $deliverWebpUnalteredLabel );?>
                          </strong>
                        </p>
                    <?php } ?>
                </li>
