...
if( $this->is_nginx ){
    ...
    $deliverAVIFLabel = __('<strong>It looks like you\'re running your site on an NGINX server. You may need additional configuration for the AVIF delivery to work as expected</strong>','shortpixel-upscale-image')." <a class=\"shortpixel-help-link\" href=\"https://shortpixel.com/knowledge-base/article/how-do-i-configure-my-web-server-to-deliver-avif-images/\" target=\"_blank\"><span class=\"dashicons dashicons-editor-help\"></span></a>";
} else {
    if( !$this->is_htaccess_writable ){
        ...
        if( $view->data->deliverWebp == 3 ){
            ...
            $deliverWebpUnalteredLabel = __('It looks like your .htaccess file cannot be written. Please fix this and then return to refresh this page to enable this option.','shortpixel-upscale-image');
        }
        ...
        $deliverWebpUnalteredLabel = '<span style="color: initial;">'. esc_html__('Based on testing your particular hosting configuration, we determined that your server','shortpixel-upscale-image').
...
<settinglist>
  ...
  <setting class='switch step-highlight-3'>
    ...
    <content>
      ...
      <info>           <?php printf(esc_html__('Generate %sWebP versions%s of images. Each image or thumbnail will use an additional credit unless you are on the %sUnlimited plan.%s','shortpixel-upscale-image'), '<a href="https://shortpixel.com/blog/how-webp-images-can-speed-up-your-site/" target="_blank">', '</a>', '<a href="https://shortpixel.com/knowledge-base/article/how-does-the-unlimited-plan-work/" target="_blank">', '</a>' );?></info>
  </setting>
  <!-- /next generation -->
  ...
  <setting class='switch step-highlight-3'>
      ...
      <content>
        <?php
          ...
          if ($avifEnabled == false)
          {
             ...
             $avifEnabledNotice .=  __('The creation of AVIF files is not possible with this license type.', 'shortpixel-upscale-image') ;
             $avifEnabledNotice .=  '<div class="spui-inline-help"><span class="dashicons dashicons-editor-help" title="Click for more info" data-link="https://shortpixel.com/knowledge-base/article/how-does-the-unlimited-plan-work/"></span></div>';
      ...
      <info>
         <?php printf(esc_html__('Generate %sAVIF versions%s of images. Each image or thumbnail will use an additional credit unless you are on the %sUnlimited plan.%s','shortpixel-upscale-image'), '<a href="https://shortpixel.com/blog/what-is-avif-and-why-is-it-good/" target="_blank">', '</a>', '<a href="https://shortpixel.com/knowledge-base/article/how-does-the-unlimited-plan-work/" target="_blank">', '</a>' );?>
      </info>
  </setting>
  <!-- // avif -->
...
<?php
  ...
  if (true === apply_filters('spui/settings/allow_cdn', true)): ?>
    ...
    if ('https://cdn.shortpixel.ai/spio' == $cdnDomain || 'https://cdn.shortpixel.ai/spui' == $cdnDomain)
    {
       $cdnDomain = 'https://spcdn.shortpixel.ai/spui';
    }
    elseif ('https://spcdn.shortpixel.ai/spio' == $cdnDomain)
    {
       $cdnDomain = 'https://spcdn.shortpixel.ai/spui';
    }
    ?>
    ...
    <info class='useCDN toggleTarget'>
           <?php printf(esc_html__('Change this only if you want to set up your %scustom domain%s.  ShortPixel CDN: %s','shortpixel-upscale-image'), '<a href="https://shortpixel.com/knowledge-base/article/how-to-serve-the-images-from-a-custom-domain/" target="_blank">', '</a>', 'https://spcdn.shortpixel.ai/spui');?>
    </info>
    ...
    <?php if($this->view->is_wpoffload)
    {
       ?>
        <warning class="cdn-offload">
          <?php _e('It looks like you have the Offload Media plugin enabled. Please note that the CDN delivery will not work because usually Offload Media is taking care of this. We recommend you to disable the CDN delivery in this case.', 'shortpixel-upscale-image'); 
          ?>
    </warning>
       <?php 
    }
...
<setting class='switch step-highlight-3'>
  <content>
   ...
   <info>
         <?php printf(esc_html__('Local delivery skips the CDN and serves next-generation files directly from your website using either the PICTURE tag method or .htaccess/nginx rules. %sRead more%s.','shortpixel-upscale-image'), '<a href="https://shortpixel.com/knowledge-base/article/which-webp-files-delivery-method-is-the-best-for-me/" target="_blank">', '</a>' );?>
   </info>
