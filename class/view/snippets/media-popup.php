...
switch($action_name)
{
	...
	case 'scale':
		...
		$suggestedFileName = $originalImage->getFileBase() . '_optimize.' . $originalImage->getExtension();
...
<div class="modal-wrapper" id="media-modal" data-item-id="<?php echo intval($this->data['item_id']) ?>" data-action-name="<?php echo esc_attr($action_name) ?>" >
    ...
    <div class="image-wrapper">
            <div class="image-original">
				...
				<span><?php _e('Before', 'shortpixel-upscale-image'); ?>
            </div>
			<div class="image-arrow">
				<i class='shortpixel-icon arrow-right'></i>
			</div>
            <div class="image-preview">
				<span><?php _e('After', 'shortpixel-upscale-image'); ?></span>
    ...
    <section class="remove action_wrapper">
		<h3><?php _e("Options", 'shortpixel-upscale-image'); ?></h3>
		<p><?php __('Note: transparency options only work with supported file formats, such as PNG', 'shortpixel-upscale-image'); ?></p>
		...
		<section class="scale action_wrapper">
			<h3><?php _e("Options", 'shortpixel-upscale-image'); ?></h3>
			<h4><?php _e('AI Image Upscale', 'shortpixel-upscale-image'); ?></h4>
			<ul>
		...
		<section class='new_file_title wrapper'>
			<span>
				<p><?php _e('New File Name', 'shortpixel-upscale-image'); ?></p>
				<input type="text" name="new_filename" value="<?php echo esc_attr($suggestedFileName) ?>">
			</span>

			<span>
				<p><?php _e('New Image Title', 'shortpixel-upscale-image'); ?></p>
	...
	<div class='button-wrapper'>
		<span>
			<button class='button' type='button' id='media-get-preview' data-action='media-get-preview'>
				...
				<?php _e('Preview','shortpixel-upscale-image'); ?>
			</button>
		</span>
		<span>
		...
		<p><strong><?php _e('A new image will be created and added to the Media Library!', 'shortpixel-upscale-image'); ?></strong></p>
	</div> <!-- button-wrapper -->
