...
<section class='panel bulk-restore' data-panel="bulk-restore"  >
  <h3 class='heading'>
	...
	<div class='bulk-special-wrapper'>
	  ...
	  <p><?php printf(esc_html__('By starting the %s bulk restore %s process, the plugin will try to restore %s all images %s to the original state. All images will become unupscaled.', 'shortpixel-upscale-image'), '<b>', '</b>', '<b>', '</b>'); ?></p>

		<p><?php printf(esc_html__('We recommend users to %s contact us %s before restoring the images - many times the restoring is not necessary and we can help. But if you choose to continue then we strongly recommend to create a full backup before starting the process.', 'shortpixel-upscale-image'), '<b><a href="https://shortpixel.com/contact" target="_blank">', '</a></b>'); ?>
		</p>
...
<section class='panel bulk-migrate' data-panel="bulk-migrate"  >
  <h3 class='heading'>
	...
	<div class='bulk-special-wrapper'>
	  ...
	  <p><?php printf(esc_html__('By starting the %s bulk metadata migration %s process, the plugin will try to migrate the old format of upscaling information (used by the plugin for versions prior to 5.0) to the new format used from version 5.0 onward for %s all the images. %s It is possible to have exceptions and some of the image information migration may fail. You should get all the details for these cases at the end of the process, in the Errors section.', 'shortpixel-upscale-image'), '<b>', '</b>', '<b>', '</b>'); ?></p>
...
<section class='panel bulk-removeLegacy' data-panel="bulk-removeLegacy"  >
  <h3 class='heading'>
	...
	<div class='bulk-special-wrapper'>
	  ...
	  <p><?php printf(esc_html__('By starting the %s remove legacy metadata %s process, the plugin will try to remove all the %s legacy data %s (that was used by the plugin to store the upscaling information in versions earlier than 5.0). If this legacy metadata isn\'t properly migrated or some of the migration failed for any reason, it will be impossible to undo or redo the process. In these cases, the upscaling information for images processed with versions earlier than 5.0 could be lost.', 'shortpixel-upscale-image'), '<b>', '</b>', '<b>', '</b>'); ?></p>
...
<section class='panel bulk-restoreAI' data-panel="bulk-restoreAI"  >
  <h3 class='heading'>
	...
	<div class='bulk-special-wrapper'>
	  ...
	  <p><?php printf(esc_html__('By starting the %s Bulk undo AI %s process, the plugin will try to revert %s all AI-generated texts %s to the original state. This will impact post and post content and metadata of your installation.  ', 'shortpixel-upscale-image'), '<b>', '</b>', '<b>', '</b>'); ?></p>
