...
<section class="panel process" data-panel="process" >
  ...
  <?php $this->loadView('bulk/part-progressbar', false, ['part' => 'process']); ?>
    ...
    <h3 class="heading">
      <?php esc_html_e('ShortPixel Bulk Upscaling is in progress','shortpixel-upscale-image'); ?>
    ...
    <?php
     if ($settings->doBackgroundProcess): ?>
      <p class="description">
        ...
        <?php
        $link = 'https://shortpixel.com/knowledge-base/article/background-processing-using-cron-jobs-in-shortpixel-image-upscaler/';
        printf(esc_html('ShortPixel Bulk is upscaling in the background. You can close this browser window now and reopen it at any time to check the status of the bulk process. %sLearn more%s','shortpixel-upscale-image'), '<strong><a href="' . esc_attr($link) . '" target="_blank">','</a></strong>'); ?>
      </p>
    <?php else: ?>
      <p class='description'>
        <?php esc_html_e('ShortPixel is upscaling your images. Please keep this window open to complete the process.', 'shortpixel-upscale-image'); ?>
      </p>
    <?php endif; ?>
    ...
    <div class='bulk-summary' data-check-visibility data-control="data-check-media-total">
      <div class='heading'>
        <span><i class='dashicons dashicons-format-image'>&nbsp;</i> <?php esc_html_e('Media Library' ,'shortpixel-upscale-image'); ?>
              <?php printf(esc_html__('( %s items )', 'shortpixel-upscale-image'), '<i data-stats-media="total">--</i>'); ?>
    ...
    <div class='image-preview-section hidden'> <!-- /hidden -->
		        ...
		        <div class="image result">
		          <img src="<?php echo esc_url(\wpSPUI()->plugin_url('res/img/bulk/placeholder.svg')); ?>" >
						<p><?php esc_html_e('Upscaled Image', 'shortpixel-upscale-image'); ?>
								- <span data-result="improvements-totalpercentage"></span>% <?php _e('smaller', 'shortpixel-upscale-image'); ?>
						</p>
						<?php $this->loadView('snippets/part-svgloader', false); ?>
		        </div>
					</div>
      ...
      <div class='ai-preview-wrapper hidden'>
          <h3><i class='shortpixel-icon ai'></i><?php _e('AI Generated Data: ', 'shortpixel-upscale-image'); ?></h3>
