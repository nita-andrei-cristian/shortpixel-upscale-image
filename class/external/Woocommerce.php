	...
	//	public function $new_sizes = array();
		...
		public function hooks()
		{
			if (\wpSPUI()->env()->plugin_active('woocommerce'))
			{
				 ...
				 // If new images are created, drop the upscale data of them . Late as possible, this is a hook often used by plugins to refine.
				 add_filter('intermediate_image_sizes_advanced', array($this, 'handleCreateImages'), 99, 3);

			}
		}

		// This hook is ran just before create new images / regenerating them. Only then signal to check for upscaled thumbs et al.
		public function signalWC()
		{
				self::$SIGNAL = true;
		}

		/** Hook to run when Wordpress is about to generate new thumbnails.  Remove backup and upscale data if that happens */
		public function handleCreateImages($new_sizes, $image_meta, $attach_id)
		{
				...
				{
						// Performance; This item is not in database, hence not upscaled in any way.
		...
		public function addWarning($tools)
		{
			 if (isset($tools['regenerate_thumbnails']) && \wpSPUI()->env()->is_autoprocess)
			 {
				  $text = $tools['regenerate_thumbnails']['desc'];
					...
					__( 'ShortPixel Image Upscaler Note:', 'shortpixel-upscale-image' ),
					__( 'The ShortPixel Image Upscaler plugin is set to automatically upscale images on upload. When running the thumbnails tools, each image that is not upscaled will be added to the queue. It is recommend to disable this option while running these tools', 'shortpixel-upscale-image')
				);
